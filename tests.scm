...
(quote (1 2 3 4 ,a))
; expect (1 2 3 4 (unquote a))

(define a 5)
(define b 5)
(let ((a 42) (b (* a 10))) (list a b))
; expect (42 50)
a
; expect 5

(cond ((and #f (/ 1 0)) 'hello)
      ((or 1) 3))
; expect 3

(define (enumerate s)
  (define (enumer lst index)
    (cond ((null? lst) nil)
          (else (cons (cons index (cons (car lst) nil)) (enumer (cdr lst) (+ 1 index)) ) )
      )
    )
    (enumer s 0)
  )
